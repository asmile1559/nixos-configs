...
let
  ...
  splits = cmd: pkgs.lib.splitString " " cmd;
  screenshot = [
    "sh"
    "-c"
    ''grim -g "$(slurp -o -r -c '#ff0000ff')" -t ppm - | satty --filename -''
  ];
in
{
  imports = [
  ...
  programs.niri = {
    ...
    settings = {
      binds = {
        # Keys consist of modifiers separated by + signs, lfollowed by an XKB key name
        ...
        "Mod+D" = {
          hotkey-overlay.title = "Open an app launcher";
        ...
        "XF86AudioMicMute" = {
          allow-when-locked = true;
          action.spawn = splits "wpctl set-mute @DEFAULT_AUDIO_SOURCE@ toggle";
        };
        ...
        "XF86AudioPlay" = {
          allow-when-locked = true;
          action.spawn = splits "playerctl play-pause";
        };

        "XF86AudioStop" = {
          allow-when-locked = true;
          action.spawn = splits "playerctl stop";
        };
      ...
      # for more about input settings, go to `https://github.com/YaLTeR/niri/wiki/Configuration:-Input`
      input = {
        keyboard = {
          xkb = { };
          numlock = true; # 开启数字锁定
      ...
      outputs = {
        ...
        "DP-1" = {
          # 使用 DP 连接的显示器
          enable = true;
          focus-at-startup = true; # 启动时 focus
          mode = {
            height = 1440;
            width = 2560;
            refresh = 165.0;
